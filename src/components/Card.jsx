...
export function Card({ id, name, username }) {
  ...
  return (
    <article className={`card ${isFavorite ? "favorite" : ""}`}>
      ...
      {!isFavorite && (
        <button className="boton-card" onClick={addFavorite}>
          Añadir a favoritos
        </button>
      )}
      {isFavorite && (
        <button className="boton-card-delete" onClick={removeFavorite}>
      ...
      {mensaje && (
        <p className={`mensaje-card ${isAdded ? "added" : ""}`}>
          {isAdded
            ? "El odontólogo se ha añadido a favoritos"
            : "El odontólogo se ha eliminado de favoritos"}
        </p>
      )}
    </article>
  );
